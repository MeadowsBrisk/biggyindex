...
export const voteCidAtom = atomWithStorage('voteCid', genCid());
export const votesAtom = atomWithStorage('votesMap', {});
export const endorsedSetAtom = atom(new Set());
export const windowBucketAtom = atom(null);
// New: persisted bucket->array store
export const endorsedBucketsStoreAtom = atomWithStorage('endorsedBuckets', {});
export const votesCacheAtom = atomWithStorage('votesCache', {}); // { bucket: { [itemId]: count } }
export const pendingEndorseAtom = atom(new Set());
export const voteLimitReachedAtom = atomWithStorage('voteLimitReached', false);
export const pendingEndorsedWithoutBucketAtom = atomWithStorage('pendingEndorsedNoBucket', []); // array of itemId strings endorsed before bucket known
export const globalEndorsedAtom = atomWithStorage('endorsedAll', []); // permanent lifetime endorsements
export const voteUsedCountsAtom = atomWithStorage('voteUsedCounts', {}); // { bucket: number }
export const voteMaxPerWindowAtom = atomWithStorage('voteMaxPerWindow', 1);
...
export const lastVoteBucketAtom = atomWithStorage('lastVoteBucket', null);
// Baselines & reconciliation (must be before endorseActionAtom & hydrateGlobalEndorsedAtom usage)
export const endorsementBaselinesAtom = atomWithStorage('endorsementBaselines', {}); // { itemId: baselineCountBeforeVote }
...
export const endorseActionAtom = atom(null, async (get, set, rawId) => {
  ...
  try {
    ...
    if (e?.message && /Failed to fetch|NetworkError/i.test(e.message)) {
      ...
      if (process.env.NEXT_PUBLIC_VOTE_DEBUG === '1') console.warn('[vote-debug][client] endorse failed, rolled back', { itemId, error: e?.message });
    }
  } finally {

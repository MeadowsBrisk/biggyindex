...
/* ImageZoomPreview */
export default function ImageZoomPreview({ imageUrl, imageUrls, alt = '', openSignal = null, hideTrigger = false, useProxy = true, onOpenChange, guardKey = null }) {
  // Collect images list
  ...
  const [pausedGif, setPausedGif] = useState({});
  const [gifDecodeRequested, setGifDecodeRequested] = useState({});
  const toggleGif = useCallback((idx) => {
    setPausedGif(p => {
      const cur = p[idx];
  ...
  const rotationFor = useCallback(i => (rotations[i] || 0) % 360, [rotations]);
  const rotate = useCallback(delta => setRotations(r => ({ ...r, [activeIndex]: (((r[activeIndex] || 0) + delta + 360) % 360) })), [activeIndex]);
  ...
  const toBase64Url = useCallback(s => { try { const utf8 = encodeURIComponent(s).replace(/%([0-9A-F]{2})/g, (_, p1) => String.fromCharCode('0x' + p1)); return btoa(utf8).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); } catch { return ''; } }, []);
  const proxify = useCallback(src => {
  ...
  useEffect(() => {
    ...
    images.filter(s => typeof s === 'string' && s && !/\.gif($|[?#])/i.test(s)).forEach(src => {
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      else if (e.key === '0') { const c = controlsRef.current[activeIndex]; c?.resetTransform?.(); c?.centerView?.(1); setRotations(r => ({ ...r, [activeIndex]: 0 })); }
          ...
          >
            ...
            <div className={cn('pointer-events-none absolute top-2 left-0 right-0 flex items-start justify-between px-2 sm:px-4 z-[10040]', 'transition-opacity duration-300', showUI ? 'opacity-100' : 'opacity-0')}>
              <div className="flex items-center gap-2 pointer-events-auto">
                <div className="flex items-center gap-1 rounded-full bg-black/35 backdrop-blur-md border border-white/10 px-2 py-1 shadow-sm">
                  ...
                  <ZoomButton icon="+" label="Zoom in" onClick={() => controlsRef.current[activeIndex]?.zoomIn?.()} />
                  <ZoomButton icon="-" label="Zoom out" onClick={() => controlsRef.current[activeIndex]?.zoomOut?.()} />
                  <ZoomButton icon="↺" label="Reset" onClick={() => { const c = controlsRef.current[activeIndex]; c?.resetTransform?.(); c?.centerView?.(1); setRotations(r => ({ ...r, [activeIndex]: 0 })); }} />
                  {currentIsGif && (
                    <button
                      type="button"
                      aria-label={isPausedActive ? 'Play GIF' : 'Pause GIF'}
            ...
            <div
              ...
              onClick={(e) => {
                ...
                try {
                  const t = e.target;
                  if (currentScaleRef.current <= 1.0001 && !t.closest('button') && !t.closest('[data-zoom-content]') && !t.closest('[data-nav]') && !t.closest('[data-thumbs]')) {
                    closePreview();
                  }
                } catch {}
              }}
            >
              ...
              <Swiper
                ...
                onSwiper={(sw) => { setSwiper(sw); if (pendingIndexRef.current != null) { try { sw.slideTo(pendingIndexRef.current, 0); } catch {} pendingIndexRef.current = null; } }}
                onSlideChange={(sw) => {
                  const idx = sw.activeIndex || 0;
                  setActiveIndex(idx);
                  try { const c = controlsRef.current[idx]; c?.resetTransform?.(); c?.centerView?.(1); } catch {}
                  setShowUI(true);
                }}
                ...
                {images.map((src, idx) => (
                  <SwiperSlide key={idx + src} className="!h-full">
                    <ZoomSlide
                      ...
                      decodeRequested={!!gifDecodeRequested[idx]}
                      decodeSrc={gifDecodeRequested[idx] ? proxify(src) : undefined}
                      onDecoded={() => {/* hook for metrics */}}
                    />
                  </SwiperSlide>
                ))}
              </Swiper>
              {total > 1 && (
                <>
                  <button data-nav aria-label="Previous image" disabled={activeIndex <= 0} onClick={() => navigate(-1)} className={cn('hidden md:flex group absolute left-0 top-0 h-full w-32 items-center justify-start pl-4 z-[10020]', activeIndex <= 0 && 'opacity-40 cursor-not-allowed')}>
                  ...
                  <button data-nav aria-label="Next image" disabled={activeIndex >= total - 1} onClick={() => navigate(1)} className={cn('hidden md:flex group absolute right-0 top-0 h-full w-32 items-center justify-end pr-4 z-[10020]', activeIndex >= total - 1 && 'opacity-40 cursor-not-allowed')}>
            ...
            <ZoomThumbnails
              images={images}
              activeIndex={activeIndex}
              onSelect={(i) => { try { swiper && swiper.slideTo(i); } catch {} setShowUI(true); }}
              proxify={proxify}

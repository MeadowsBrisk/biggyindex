...
import ThumbIcon from '@/app/assets/svg/thumb.svg';
 ...
 */
export default function EndorseButton({ itemId, onHydrated, compact = false }) {
  const id = String(itemId);
  // Narrow subscriptions to just this item's vote count & endorsed membership
  const voteCountAtom = React.useMemo(() => selectAtom(votesAtom, (v) => v[id] || 0), [id]);
  ...
  const baselineAtom = React.useMemo(() => selectAtom(endorsementBaselinesAtom, (b) => b[id] || 0), [id]);
  ...
  const tooltip = endorsed
    ? 'Already endorsed'
    : disabledForLimit
      ? (limitReached ? "You've used today's endorsement" : 'One per day')
      : 'Endorse (one per day)';

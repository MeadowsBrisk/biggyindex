...
// Use tuple for ease to satisfy framer-motion types
...
const wrapperVariants: Variants = {
  ...
  show: (r: boolean) => r 
    ? { opacity: 1, y: 0 } 
    : { opacity: 1, y: 0, transition: { duration: 0.45, ease: [0.25, 0.6, 0.3, 1] as BezierEasing } }
};
...
interface BraceCustom {
  side: 'left' | 'right';
  reduce: boolean;
}

const braceVariants: Variants = {
  hidden: (c: BraceCustom) => ({ opacity: 0, x: 0, rotate: c.side === 'left' ? -8 : 8, scale: 0.92 }),
  show: (c: BraceCustom) => c.reduce ? {
    opacity: 1,
    x: c.side === 'left' ? -9 : 9,
    rotate: 0,
    scale: 1
    ...
    rotate: 0,
    scale: 1,
    transition: { duration: 0.55, ease: [0.25, 0.6, 0.3, 1] as BezierEasing }
  }
};

const titleContainer: Variants = {
  hidden: {},
  show: (r: boolean) => r ? { transition: { staggerChildren: 0 } } : { transition: { staggerChildren: 0.045, delayChildren: 0.18 } }
};
...
const charVariant: Variants = {
  ...
  show: (r: boolean) => r 
    ? { opacity: 1, y: 0 } 
    : { opacity: 1, y: 0, rotateX: 0, transition: { duration: 0.42, ease: 'easeOut' as const } }
};
...
export default function AnimatedLogoHeader({ className = '', rightSlot = null }: AnimatedLogoHeaderProps) {
  ...
  React.useEffect(() => {
    ...
    if (reduce) {
      ...
      braceControls.start((custom: BraceCustom) => ({
        opacity: 1,
      ...
      <Link href={homeHref} className="relative inline-flex h-10 w-7 items-center justify-center focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/70 rounded-md" aria-label="Go to homepage">
        ...
        >
          <motion.span
            ...
            custom={{ side: 'left', reduce: !!reduce }}
            initial="hidden"
          ...
          <motion.span
            className="text-[1.9em] text-gray-900 dark:text-gray-100"
            variants={braceVariants}
            custom={{ side: 'right', reduce: !!reduce }}
            initial="hidden"
            animate={braceControls}
            style={{ fontWeight: 600, position: 'absolute' }}
          >{'}'}</motion.span>

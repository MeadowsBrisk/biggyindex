...
export default function PriceRange() {
  ...
  const clamp = (val: number | null | string | undefined, lo: number, hi: number): number => {
  ...
  const commit = useCallback((minVal: number, maxVal: number, isUser = false) => {
  ...
  const toView = (gbp: number): number => {
    const n = Number(gbp);
    if (!Number.isFinite(n)) return 0;
    return isUSD && usdRate ? Math.round(n * usdRate) : n;
  };
  
  const toGBP = (viewVal: number | string): number => {
    const n = Number(viewVal);
    if (!Number.isFinite(n)) return 0;
    return isUSD && usdRate ? (n / usdRate) : n;
  };
      ...
      <div className="flex items-end justify-between gap-3 pt-1">
        <div className="flex-1 min-w-0">
          ...
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => onMin(e.target.value)}
          />
        </div>
        ...
        <div className="flex-1 min-w-0 text-right">
          ...
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => onMax(e.target.value)}
          />
        </div>
      </div>

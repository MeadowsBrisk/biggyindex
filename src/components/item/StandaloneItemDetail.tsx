...
export default function StandaloneItemDetail({ baseItem, detail, unavailable, market }: StandaloneItemDetailProps) {
  ...
  const sl = (baseItem as any)?.sl || (detail as any)?.sl || (detail as any)?.share?.shortLink || (baseItem as any)?.url || (detail as any)?.url || (refNum ? `https://littlebiggy.org/item/${refNum}/view/p` : null);
  const lbGuideClick = useLBGuideGate(sl, { id: String(refNum || ''), name: (baseItem as any)?.n || '', market, category: category || undefined });

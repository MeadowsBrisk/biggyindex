...
export default function InfoButton({ content }) {
  ...
  const [showAppreciation, setShowAppreciation] = React.useState(false); // controls toast visibility
  const [appreciationShown, setAppreciationShown] = React.useState(false); // has toast ever been shown
  const appreciationTimeoutRef = React.useRef(null);
  ...
  const handleCopy = async () => {
    try {
      ...
      if (!appreciationShown) {
        ...
        clearTimeout(appreciationTimeoutRef.current);
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-stretch rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-800 focus-within:ring-2 focus-within:ring-emerald-400/50">
          ...
          <input
            ...
            onClick={(e) => { e.currentTarget.select(); handleCopy(); }}

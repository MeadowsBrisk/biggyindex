...
const cache = new Map(); // id -> { data?, promise?, error? }

export function getCachedSellerDetail(id) {
  const key = String(id || '');
  return cache.get(key)?.data || null;
}

export function loadSellerDetail(id) {
  const key = String(id || '');
...
export function prefetchSellerDetail(id) { try { return loadSellerDetail(id); } catch { return Promise.resolve(null); } }
export function clearSellerDetail(id) { cache.delete(String(id || '')); }

...
export const handler: Handler = async (event) => {
  ...
  const log = (m: string) => console.log(`[crawler:index-de] ${m}`);
  const warn = (m: string) => console.warn(`[crawler:index-de] ${m}`);
  const errlog = (m: string) => console.error(`[crawler:index-de] ${m}`);

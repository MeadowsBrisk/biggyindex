...
export const handler: Handler = async (event) => {
  ...
  const log = (m: string) => console.log(`[crawler:index-it] ${m}`);
  const warn = (m: string) => console.warn(`[crawler:index-it] ${m}`);
  const errlog = (m: string) => console.error(`[crawler:index-it] ${m}`);

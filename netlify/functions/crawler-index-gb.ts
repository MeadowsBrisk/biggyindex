...
export const handler: Handler = async (event) => {
  ...
  const log = (m: string) => console.log(`[crawler:index-gb] ${m}`);
  const warn = (m: string) => console.warn(`[crawler:index-gb] ${m}`);
  const errlog = (m: string) => console.error(`[crawler:index-gb] ${m}`);

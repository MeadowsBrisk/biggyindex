...
export const handler: Handler = async (event) => {
  ...
  const log = (m: string) => console.log(`[crawler:index-fr] ${m}`);
  const warn = (m: string) => console.warn(`[crawler:index-fr] ${m}`);
  const errlog = (m: string) => console.error(`[crawler:index-fr] ${m}`);

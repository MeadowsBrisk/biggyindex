...
export const handler: Handler = async (event) => {
  ...
  const log = (m: string) => console.log(`[crawler:index-pt] ${m}`);
  const warn = (m: string) => console.warn(`[crawler:index-pt] ${m}`);
  const errlog = (m: string) => console.error(`[crawler:index-pt] ${m}`);

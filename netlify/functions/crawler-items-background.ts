...
export const handler: Handler = async (event) => {
  ...
  const { log, warn, error: err } = createFnLogger('crawler:items');
  ...
  log(`worklist unique=${wl.uniqueIds.length} toCrawl=${wl.toCrawl.length} already=${wl.alreadyHave.length}`);
    ...
    // Plan modes via shared staleness logic (matches CLI behaviour)
    const { planItemModes } = await import('../../scripts/unified-crawler/stages/items/planModes');
    const { planned: rawPlan, indexChangedCount, noFullCrawlCount } = await planItemModes({
      uniqueIds: wl.uniqueIds,
      presenceMap: wl.presenceMap,
      idLua: wl.idLua,
      sharedStoreName: env.stores.shared,
    });
    if (indexChangedCount > 0) log(`index changes detected: ${indexChangedCount}`);
    if (noFullCrawlCount > 0) log(`items missing lastFullCrawl: ${noFullCrawlCount}`);

    // Sort: full-crawl items first so they get priority within the time budget
    const plan = [...rawPlan].sort((a, b) => (a.mode === 'full' ? 0 : 1) - (b.mode === 'full' ? 0 : 1));
  ...
  const runOne = async (it: { id: string; markets: import("../../scripts/unified-crawler/shared/types").MarketCode[]; mode: "full" | "reviews-only" }) => {
      ...
      try {
        const res = await processSingleItem(
          ...
          { client: httpClient, logPrefix: "[crawler:items]", mode: it.mode, indexLua: wl.idLua.get(it.id) || undefined, sharesAgg, forceShare: refreshShare, indexEntry: wl.indexEntryById?.get(it.id) }
        );
